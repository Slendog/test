...
export default new class Nyaa {
  base = 'https://nyaa.si/'
  category = '1_2' // Anime - English-translated. Use '1_0' for all anime.
  ...
  async test () {
    const res = await fetch(`${this.base}?page=rss&q=one+piece`)
    if (!res.ok) throw new Error(`Nyaa unreachable (HTTP ${res.status}). The site may be down or blocked by your ISP.`)
    return true
  }
}()
